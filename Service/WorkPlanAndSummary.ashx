<%@ WebHandler Language="C#" Class="WorkPlanAndSummary" %>
...
/// 覆盖问题征集
/// </summary>
public class WorkPlanAndSummary : IHttpHandler, IRequiresSessionState
{
    HttpRequest Request;
    ...
    /// 保存工作总结
    /// </summary>
    public void SaveWorkSummary()
    {
        //判断上个月的工作总结是否已录入
        List<SqlParameter> _paras = new List<SqlParameter>();
        _paras.Add(new SqlParameter("@deptid", Session["DeptID"]));
        _paras.Add(new SqlParameter("@workmonth", DateTime.Now.AddMonths(-1).ToString("yyyy-MM")));
        ...
        {
            Response.Write("{\"success\":false,\"msg\":\"" + DateTime.Now.AddMonths(-1).ToString("yyyy年MM月") + "工作总结已录入！\"}");
            return;
        }
        //获取数据行数
        int rowsCount = 0;
    ...
    public void SaveWorkPlan()
    {
        ...
        List<SqlParameter> _paras = new List<SqlParameter>();
        _paras.Add(new SqlParameter("@deptid", Session["DeptID"]));
        _paras.Add(new SqlParameter("@workmonth", DateTime.Now.ToString("yyyy-MM")));
        ...
        if (ds.Tables[0].Rows.Count > 0)
        {
            Response.Write("{\"success\":false,\"msg\":\"" + DateTime.Now.ToString("yyyy年MM月") + "工作计划已录入！\"}");
            return;
        }
        //获取数据行数
        int rowsCount = 0;
