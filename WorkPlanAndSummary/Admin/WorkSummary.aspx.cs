...
public partial class Admin_WorkSummary : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            {
                DataSet ds1 = DirectDataAccessor.QueryForDataSet("select workmonth from DeptWorkSummary group by workmonth order by workmonth desc ");
                ...
                /////
               //默认显示上一月
                BindRep(DateTime.Now.AddMonths(-1).ToString("yyyy-MM"));
                thisMonth = DateTime.Now.AddMonths(-1).ToString("yyyy年MM月");
            }
        }
    ...
    private void BindRep(string ym)
    {
        ...
        sql.Append(" SELECT deptid FROM DeptWorkSummary  WHERE WorkMonth=@workmonth GROUP BY deptid ");
    ...
    public string handleHasSp(string deptid, string workmonth, string hassp)
    {
        ...
        if (hassp == "0")//未录入
        {
            str = string.Format("<a href=\"javascript:void(0);\" title=\"点击补录工作总结\" class=\"easyui-tooltip\" onclick=\"addSummary('{0}','{1}');\"><b style='color:red;'>未录入</b></a>", deptid, workmonth);
        }
        else
        {
            str = "已录入";
